package com.transcendence.universe.abp.classic.fringerprint.core;
...
public class FingerprintCore {
    ...
    public FingerprintCore(Context context) {
        ...
        Logs.logE("fingerprint isSupport: " + isSupport);
        initCryptoObject();
    }
    ...
    private void initCryptoObject() {
        try {
            ...
            Logs.logE("create cryptoObject failed!");
        }
    }
    ...
    private void notifyAuthenticationSucceeded() {
        Logs.logE("onAuthenticationSucceeded");
        mFailedTimes = 0;
    ...
    private void notifyAuthenticationError(int errMsgId, CharSequence errString) {
        Logs.logE("onAuthenticationError, errId:" + errMsgId + ", err:" + errString + ", retry after 30 seconds");
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void notifyAuthenticationFailed(int msgId, String errString) {
        Logs.logE("onAuthenticationFailed, msdId: " +  msgId + " errString: " + errString);
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void onFailedRetry(int msgId, String helpString) {
        mFailedTimes++;
        Logs.logE("on failed retry time " + mFailedTimes);
        if (mFailedTimes > 5) { // 每个验证流程最多重试5次，这个根据使用场景而定，验证成功时清0
            Logs.logE("on failed retry time more than 5 times");
            return;
        }
        Logs.logE("onFailedRetry: msgId " + msgId + " helpString: " + helpString);
        cancelAuthenticate();
    ...
    public static FingerprintManager getFingerprintManager(Context context) {
        ...
        try {
            ...
            Logs.logE("have not class FingerprintManager");
        }
